...
- **CSRF por header**: `GET /api/csrf-token` devolve `{token}`; mutações enviam
  `X-CSRF-Token`. Único caminho isento é `/health`.
...
| **Admin · Configurações** | `/api/admin` | `GET/PUT /configuracoes` (por categoria) |
...
- `BASE_URL` — usada nos links de e-mail (apontam para o SPA, ex. redefinição de senha).
- `SPA_DIST_PATH` — caminho do build do React em produção (default `../frontend/dist`).
- `RESEND_*` (e-mail transacional).
