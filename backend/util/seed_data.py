...
# E-mails de todas as contas demo (empresas + motoristas). Fonte única usada
# pela rotina de auto-cura de senhas (_reparar_senhas_demo). Mantém em sincronia
# com empresas_demo/motoristas_demo abaixo.
EMAILS_EMPRESAS_DEMO = (
    "contato@andradelog.com.br",  # âncora
    "logistica@agrocerrado.com.br",
    "contato@belavistalat.com.br",
    "frete@moveisexpress.com.br",
    "operacoes@distribuidoravale.com.br",
)
EMAILS_MOTORISTAS_DEMO = (
    "carlos.souza@email.com.br",
    "marcos.lima@email.com.br",
    "rafael.dias@email.com.br",
    "andre.nogueira@email.com.br",
    "jonatas.campos@email.com.br",
    "eduardo.rocha@email.com.br",
)
EMAILS_DEMO = EMAILS_EMPRESAS_DEMO + EMAILS_MOTORISTAS_DEMO
...
def _reparar_senha_demo(email: str) -> bool:
    """
    Auto-cura a senha de UMA conta demo, se necessário.

    Repara apenas quando a senha gravada NÃO confere com SENHA_PADRAO_SEED
    (hash vazio/inválido/drifted de um seed parcial anterior). Não toca em
    contas cuja senha já confere (evita reescrita desnecessária / clobber).

    Returns:
        True se a senha foi redefinida; False se a conta não existe ou já
        estava correta.
    """
    ...
    senha_atual = usuario.senha or ""
    try:
        senha_confere = bool(senha_atual) and verificar_senha(
            SENHA_PADRAO_SEED, senha_atual
        )
    except Exception:
        # Hash malformado/incompatível com bcrypt -> tratar como drifted.
        senha_confere = False

    if senha_confere:
        return False

    if usuario_repo.atualizar_senha(usuario.id, criar_hash_senha(SENHA_PADRAO_SEED)):
        logger.info(f"↺ Senha demo reparada para {email} (ID: {usuario.id})")
        return True
    return False


def reparar_senhas_demo() -> int:
    """
    Varre todas as contas demo conhecidas e repara senhas drifted/inválidas.

    Idempotente e barato: só executa UPDATE nas contas cuja senha não confere
    com SENHA_PADRAO_SEED. Roda a cada boot de dev (antes da guarda de
    idempotência dos dados pesados), tornando o seed auto-curável mesmo quando
    a empresa âncora já existe mas há contas demo com senha quebrada.

    Returns:
        Quantidade de senhas reparadas.
    """
    reparadas = 0
    for email in EMAILS_DEMO:
        if _reparar_senha_demo(email):
            reparadas += 1
    if reparadas:
        logger.info(f"Auto-cura de senhas demo: {reparadas} conta(s) reparada(s).")
    return reparadas
...
def carregar_girochoffer_demo():
    ...
    # -- Auto-cura de senhas (roda SEMPRE, antes da guarda de idempotência) --
    # Repara contas demo cuja senha não confere com SENHA_PADRAO_SEED (ex.:
    # seed parcial anterior gravou hash vazio/inválido). É leve e idempotente:
    # só faz UPDATE nas contas drifted, sem duplicar dados pesados nem mexer
    # em contas já corretas. Garante que os logins demo funcionem mesmo quando
    # a empresa âncora já existe (a guarda abaixo curto-circuitaria o resto).
    reparar_senhas_demo()
    ...
    for ref, dados in zip(refs_empresa, empresas_demo):
        ...
        if usuario_existente is not None:
            ...
            # Reaproveita o usuário existente, mas garante senha demo válida.
            _reparar_senha_demo(dados["email"])
        else:
            usuario_id = usuario_repo.inserir(Usuario(
                id=0,
                nome=dados["nome"],
