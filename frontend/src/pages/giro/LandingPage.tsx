...
/* Landing pública do GiroChoffer (rota /). Portada fielmente de
   design/girochoffer-react/src/pages/Landing.jsx, mas ligada à infra de
   produção: react-router-dom, authStore (sessão), toast (uiStore), tokens
   de tema e estilos inline (sem Bootstrap). Não exige autenticação. */

const HERO_IMG = '/static/uploads/seed/hero.jpg'
...
export default function LandingPage() {
  ...
  /* Destino do painel quando já há sessão (empresa/motorista). */
  const painel = isEmpresa ? '/empresa' : isMotorista ? '/motorista' : null

  const irPainel = () => {
    if (painel) navigate(painel)
  }

  const inscrever = () => {
    const parsed = emailSchema.safeParse(email)
    if (!parsed.success) {
      toast.erro(parsed.error.issues[0]?.message ?? 'E-mail inválido.')
      return
    }
    setEmail('')
    toast.sucesso('Inscrição realizada! Você receberá nossas novidades.')
  }
    ...
    <div style={{ minHeight: '100vh', background: colors.bg, fontFamily: fonts.body, color: colors.text }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: '18px', lineHeight: 1.6, color: '#4A5462', margin: '0 0 32px', maxWidth: '520px' }}>
            O GiroChoffer aproxima empresas que precisam transportar cargas e motoristas que buscam fretes — de
            forma simples, direta e sem intermediários.
          </p>
          <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
            <Button
              onClick={() => navigate('/entrar?papel=empresa')}
              style={{
                padding: '15px 30px',
                background: colors.primary,
                color: '#fff',
            ...
            <Button
              onClick={() => navigate('/entrar?papel=motorista')}
              style={{
                padding: '15px 30px',
              ...
              hoverStyle={{ background: colors.navyHover }}
            >
              Sou motorista
            </Button>
          </div>
          <div style={{ display: 'flex', gap: '32px', marginTop: '40px' }}>
            {stats.map(([v, l]) => (
              <div key={l}>
                <div style={{ fontFamily: fonts.heading, fontWeight: 800, fontSize: '26px', color: colors.navy }}>
                  {v}
                </div>
        ...
        {heroErro ? (
          <div
            style={{
              aspectRatio: '4 / 3',
              borderRadius: '18px',
              background: 'repeating-linear-gradient(135deg,#E9EEF4 0 14px,#EFF3F7 14px 28px)',
          ...
          <img
            src={HERO_IMG}
            alt="Caminhão na estrada transportando carga"
            onError={() => setHeroErro(true)}
            style={{
              aspectRatio: '4 / 3',
              width: '100%',
              objectFit: 'cover',
      ...
      <section style={{ background: '#fff', borderTop: `1px solid ${colors.border}`, borderBottom: `1px solid ${colors.border}` }}>
        <div style={{ maxWidth: '1160px', margin: '0 auto', padding: '60px 32px' }}>
          ...
          <p style={{ textAlign: 'center', color: colors.muted, fontSize: '16px', margin: '0 0 44px' }}>
            Do anúncio da carga ao frete concluído em quatro passos.
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '24px' }}>
            {steps.map((step) => (
              <div key={step.n} style={{ textAlign: 'center', padding: '0 8px' }}>
                ...
                <div style={{ fontWeight: 700, fontSize: '16px', color: colors.ink, marginBottom: '6px' }}>
                  {step.t}
                </div>
      ...
      >
        <div style={{ background: '#fff', border: `1px solid ${colors.border}`, borderRadius: '16px', padding: '34px' }}>
          ...
          >
            Encontre quem transporta sua carga
          </h3>
          {benefEmpresa.map((b) => (
        ...
        <div style={{ background: colors.navy, borderRadius: '16px', padding: '34px', color: '#fff' }}>
          ...
          <h3
            style={{ fontFamily: fonts.heading, fontWeight: 800, fontSize: '24px', color: '#fff', margin: '0 0 18px' }}
          >
            Frete na sua rota, sem complicação
          </h3>
      ...
      <section style={{ background: '#fff', borderTop: `1px solid ${colors.border}` }}>
        <div style={{ maxWidth: '760px', margin: '0 auto', padding: '56px 32px', textAlign: 'center' }}>
          ...
          >
            Receba novidades e fretes em alta
          </h3>
          <p style={{ color: colors.muted, margin: '0 0 24px' }}>
            Cadastre seu e-mail e fique por dentro das novidades do GiroChoffer.
          </p>
      ...
      <footer style={{ background: colors.navyDeep, color: '#9AA7B6' }}>
        ...
        >
          ...
          <div style={{ fontSize: '13px' }}>
            © {year} GiroChoffer · Marketplace de fretes · Termos de uso · Privacidade
          </div>
        </div>
      </footer>
    </div>
